...
const MNEMO = {
  ...
  "な":"A knot tied by a Nun.","に":"A NEEdle beside two threads.","ぬ":"NOOdles tangled in a loop.","ね":"A NEko (cat) with a curled tail.","の":"A NO-entry sign swirl.",
...
(function(){
  for (const row of HIRA_ROWS){
    for (let i=0;i<row.ks.length;i++){
      ...
      if (row.base) hint = "Take " + row.ks[i].normalize('NFD')[0].normalize('NFC') +
        " and voice it: " + (KANA_SPECIAL[k] || ROW_HINT[row.r] + ", plus the " + vowel + " vowel");
...
const VOCAB = {};
function V(jp,kj,en,e,lv,cat){ VOCAB[jp] = {jp,kj,en,e,lv,cat}; }
// -- greetings & basics (N5)
...
V("じ","字","character / letter","🔤",4,"n4noun");
...
V("おれい","お礼","thanks / reward","💝",4,"n4noun");
...
// N5 (~85)
K("一","いち・ひと","one","いち"); K("二","に・ふた","two","に"); K("三","さん・みっ","three","さん");
K("四","よん・し","four","よん"); K("五","ご・いつ","five","ご"); K("六","ろく・むっ","six","ろく");
...
K("長","ちょう・なが","long / chief","ながい"); K("多","た・おお","many","たくさん");
...
K("語","ご・かた","language","ことば"); K("外","がい・そと","outside","そと");
K("上","じょう・うえ","up / above","うえ"); K("下","か・した・へた","down / below","へた");
...
K("手","しゅ・て","hand","て"); K("目","もく・め","eye","め");
...
K("考","こう・かんが","consider","おもう"); K("知","ち・し","know","しらべる");
...
K("教","きょう・おし","teach","おしえる"); K("室","しつ","room","へや");
...
K("遠","えん・とお","far","とおい"); K("速","そく・はや","fast","はやい");
...
K("森","しん・もり","forest","もり"); K("林","りん・はやし","grove","もり");
...
K("字","じ","character","じ"); K("文","ぶん・もん","sentence / text","ぶんぽう");
...
Gr("g_ni","〜に",
  ...
  [["がっこうに いきます。","I go to school."],["七時に おきます。","I wake up at seven."]]);
Gr("g_de","〜で",
...
Gr("g_counter","かぞえかた",
  ...
  "ものは「〜つ」、ひとは「〜人」、おかねは「〜円」で かぞえます。",
  [["りんごを みっつ ください。","Three apples, please."],["こどもが 三人 います。","There are three children."]]);
Gr("g_yori","〜より 〜のほうが",
...
Gr("g_nagara","〜ながら", null,
  "ふたつの ことを どうじに する。ます形の stem＋「ながら」。",
...
Gr("g_meirei","命令形・意向形", null,
  "命令形は つよい めいれい：「【行|い】け！」。意向形は「〜しよう」：「【行|い】こう」。",
